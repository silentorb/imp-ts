...
| `nodePropertiesJson` | `(alias: string) => string` | may — SQL expression for a node row's JSON property bag used as the left side of traverse `json_patch` (default: `{alias}.properties`) |
