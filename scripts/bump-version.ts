#!/usr/bin/env bun
/**
 * Bump an imp/tome workspace package version and update dependent workspace:^ ranges.
 * Agent chooses bump level (minor | patch) based on change analysis.
 *
 * Usage:
 *   bun scripts/bump-version.ts <package-name> <minor|patch> [--install]
 *   bun scripts/bump-version.ts --baseline [--install]   # workspace:* → workspace:^X.Y.0 for all internal deps
 */
...
function allPackages(): PackageEntry[] {
  const roots = [packagesDir, ...extraPackageRoots];
  return roots.flatMap(loadPackages);
}
...
function updateRangesForDependency(pkg: PackageEntry, depName: string, depVersion: string): boolean {
  ...
  for (const section of depSections(pkg.manifest)) {
    ...
    const next = range;
    if (section[depName] !== next) {
      section[depName] = next;
      changed = true;
    }
  }
  return changed;
...
function lockfileHint(changedImp: boolean): void {
  console.log("\nLockfiles to refresh:");
  console.log("  - .mnt/imp-ts  → bun install");
  if (changedImp || repoRoot.includes("tome")) {
